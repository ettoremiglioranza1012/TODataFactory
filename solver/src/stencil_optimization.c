...
void top3dmgcg(const uint_fast32_t nelx, const uint_fast32_t nely,
  ...
  // Compute ndof for the force vector
  const int ndof =
      3 * gridContext.wrapx * gridContext.wrapy * gridContext.wrapz;

  // Load initialization: check for external load file or use hardcoded fallback
  if (load_file_path != NULL) {
    // ========================================
    // Binary file ingestion for distributed loads
    // ========================================
    FILE *load_file = fopen(load_file_path, "rb");
    if (load_file == NULL) {
      fprintf(stderr, "❌ Error: Could not open load file: %s\n",
              load_file_path);
      exit(EXIT_FAILURE);
    }

    // Optional: Verify file size matches expected ndof
    fseek(load_file, 0, SEEK_END);
    long file_size = ftell(load_file);
    long expected_size = (long)ndof * sizeof(double);
    fseek(load_file, 0, SEEK_SET);

    if (file_size != expected_size) {
      fprintf(stderr, "❌ Error: Load file size mismatch.\n");
      fprintf(stderr, "   Expected: %ld bytes (%d DOFs × %zu bytes/double)\n",
              expected_size, ndof, sizeof(double));
      fprintf(stderr, "   Got: %ld bytes\n", file_size);
      fclose(load_file);
      exit(EXIT_FAILURE);
    }

    // Read binary data directly into F array
    // Note: F is CTYPE, we read as double and may need conversion
    double *temp_buffer = malloc(sizeof(double) * ndof);
    size_t items_read = fread(temp_buffer, sizeof(double), ndof, load_file);
    fclose(load_file);

    if (items_read != (size_t)ndof) {
      fprintf(stderr, "❌ Error: Failed to read all DOFs from load file.\n");
      fprintf(stderr, "   Expected: %d, Read: %zu\n", ndof, items_read);
      free(temp_buffer);
      exit(EXIT_FAILURE);
    }

    // Copy to F array (handles potential CTYPE/double mismatch)
#pragma omp parallel for schedule(static)
    for (int i = 0; i < ndof; i++) {
      F[i] = (CTYPE)temp_buffer[i];
    }
    free(temp_buffer);

    printf("📁 Loads: Read %d DOFs from file: %s\n", ndof, load_file_path);

  } else {
    // ========================================
    // Fallback: Hardcoded cantilever load
    // ========================================
    printf("📁 Loads: Using hardcoded cantilever distributed load\n");
...
#pragma omp parallel for schedule(static) reduction(max : change)
  ...
  writeDensityBinaryFile(nelx, nely, nelz, xPhys, "density.bin");
