...
// writes density field to binary file for ML pipeline
// Format: sequential array of doubles (nelx * nely * nelz)
void writeDensityBinaryFile(const int nelx, const int nely, const int nelz,
                            const DTYPE *densityArray, const char *filename) {
  const int paddingy =
      (STENCIL_SIZE_Y - ((nely + 1) % STENCIL_SIZE_Y)) % STENCIL_SIZE_Y;
  const int paddingz =
  ...
  if (fid == NULL) {
    fprintf(stderr, "Error: Could not open binary density file: %s\n",
            filename);
    return;
  }

  // Write density values for actual domain (excluding padding/halo)
  // Match the indexing from writeDensityVtkFile
  for (unsigned int i1 = 1; i1 < nelx + 1; i1++) {
    for (unsigned int k1 = 1; k1 < nelz + 1; k1++) {
      for (unsigned int j1 = 1; j1 < nely + 1; j1++) {
        const uint64_t idx = i1 * elWrapy * elWrapz + k1 * elWrapy + j1;
        double value = (double)densityArray[idx];
        fwrite(&value, sizeof(double), 1, fid);
      }
    }
  }
